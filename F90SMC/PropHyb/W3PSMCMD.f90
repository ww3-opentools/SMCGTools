...
!
      ...
      Integer :: icl,jcl,ivf,jvf,iuf,juf,LvR
...
!$acc data create(cxtot,cytot,FCNt,AFCN,BCNt,UCFL,VCFL,CQA,CXTOT,CYTOT,FUMD,FUDIFX,ULCFLX,FVMD,FVDIFY,VLCFLY,cq)

!$acc data present(rcela,cgrp,cx,cy,angcd,clats,isd,jsd,field) 
!$acc kernels 
...
!Li   is undefined there.  Its U-component is never used. 
...
!
!$acc data present (hcel,Wnmk,VA,CTHG0S,CGrp,DCYDX,DCXDY,DCXDX,DCYDY,ESC,EC2,ES2, & 
!$acc               DHDX,ECOS,DHLMT,dsip,cx,cy,DHDY,esin,sig)
!$ACC kernels 
!$ACC Loop independent private(frk, frg, fkc, vq, vcflt, dm, db, sigsnh, cflk, DDNorm) 

!$OMP Parallel DO Default(Shared), Private(FRK,FRG,FKC,VQ,VCFLT,DM,DB,SIGSNH,CFLK),  &
!$OMP             Private(ITH,IK,ISP,ISEA,JSEA,FGC,FKD,FKS,DEPTH30,DDNorm)

 LocLop: DO JSEA=1, NSEAL

!Li   Work out full grid ISEA from local JSEA index.
...
!$acc data present(cf,uc,isd,ice,clats,fu,fx,uflx)
!$acc kernels
...
!  Note polar cell (if any) AUN = 0.0 as it has no U-face.
           GrdX(n)=DX0I*AUN(n)/( CLats(n)*ICE4(n) )
...
!$acc data create(avn,aun,cvf)
!$acc data present(ice,isd,jsd)
!$acc kernels
...
!$OMP Parallel Default(Shared), Private(i,j,k,L,M,N),  &
!$OMP Private(CNST,CNST1,CNST2,CNST3,CNST4,CNST5,CNST6)
...
!Li
       ...
       SUBROUTINE SMCGtCrfr(CoRfr, SpeTHK)
         ...
         INTEGER ::  NRefr
         REAL:: CNST, CNST0, CNST1, CNST2, CNST3, CNST4, CNST5, CNST6
         REAL, DIMENSION(NDir) :: Spectg, SpeGCT
         Integer :: i,j,k,l,m,n
!$ACC Routine SEQ
!$ !Li   Rotation is done for all frequency bins at each frequency so
!$ !Li   the frequency loop can be parallelised.  JGLi16Nov2017

!$ !Li  OpenMP parallelization is moved up to ISEA loop.  JGLi03Oct2019
! !$OMP Parallel Default(Shared), Private(j, K, L, M, n),  &
! !$OMP Private(CNST,CNST0,CNST1,CNST2,CNST3,CNST4,CNST5,CNST6), &
! !$OMP Private(Spectg, SpeGCT) 
...
!!   Divid the j bin energy by fraction of CNST6 and store in SpeGCT
           SpeGCT(L)=SpeGCT(L)+Spectg(j)*CNST2
           SpeGCT(M)=SpeGCT(M)+Spectg(j)*CNST1
...
!$ !Li  OpenMP parallelization is moved up to ISEA loop.  JGLi03Oct2019
! !$OMP Parallel Default(Shared), Private(j, L, M, n),  &
...
!$acc data present ( cx,cy,DCXDX,DCXDY,DCYDY,DCYDX,ANGCD) create(cxcy,GrHx,GrHy)
!$acc kernels
...
!/
      ...
      REAL,    INTENT(OUT)    :: FIELD(NSEA)
!/
!/ ------------------------------------------------------------------- /
!/ Local parameters
!/
      INTEGER :: STATUS(MPI_STATUS_SIZE,NSPEC),  &
                 IOFF, IXY, IS0, IB0, NPST 
      INTEGER :: ISEA, JSEA, J 
!/
!/ ------------------------------------------------------------------- /
!/
...
!
      IF ( BSTAT(IBFLOC) .EQ. 2 ) THEN
          ...
          IF ( NRQSG2 .GT. 0 ) CALL MPI_WAITALL(                 &
               NRQSG2, IRQSG2(IOFF,2), STATUS, IERR_MPI )
          BSTAT(IBFLOC) = 0
        END IF
!
...
!
      IF ( BSTAT(IBFLOC) .EQ. 0 ) THEN
          ...
          IF ( NRQSG2 .GT. 0 ) CALL MPI_STARTALL (               &
               NRQSG2, IRQSG2(IOFF,1), IERR_MPI )
        END IF
!
! 2.c Put local spectral densities in store
!
      DO JSEA=1, NSEAL
        GSTORE(IAPROC+(JSEA-1)*NAPROC,IBFLOC) = VA(ISPEC,JSEA)
        END DO
!
...
!
      IOFF =  1 + (BISPL(IBFLOC)-1)*NRQSG2
      IF ( NRQSG2 .GT. 0 ) CALL MPI_WAITALL (                    &
           NRQSG2, IRQSG2(IOFF,1), STATUS, IERR_MPI )
!
...
!
!/ ------------------------------------------------------------------- /
      SUBROUTINE W3SCATSMC ( ISPEC, FIELD )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH-III           NOAA/NCEP |
!/                  |           H. L. Tolman            |
...
!/
      INTEGER, INTENT(IN)     :: ISPEC 
      REAL,    INTENT(IN)     :: FIELD(NSEA)
!/
!/ ------------------------------------------------------------------- /
!/ Local parameters
!/
      INTEGER     :: IXY, IOFF, IB0, STATUS(MPI_STATUS_SIZE,NSPEC) 
      INTEGER     :: ISEA, JSEA, J
      LOGICAL     :: DONE
!/
!/ ------------------------------------------------------------------- /
...
!
      DO JSEA=1, NSEAL
        ISEA   = MIN( IAPROC+(JSEA-1)*NAPROC, NSEA )
        VA(ISPEC,JSEA) = SSTORE(ISEA,IBFLOC)
        END DO
!
