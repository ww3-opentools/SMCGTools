...
!!                    J G Li   12 Jul 2019
!! Add ACC directives for GPU apllications by Dave Norton. 26Sep2019
!! Bug fix for consistency with CPU Hybrid application.  JGLi08Sep2019
!! Finish WW3 spectral memory arrangement and MPI calls. JGLi28Oct2019
!! Update Arctic part for WW3 memory arrangement.  JGLi05Nov2019
!!
      ...
      PROGRAM SMCPrDRG 
       ...
       REAL (Kind=8) :: TM1, TM2, GTM0, GTM1, GTM2
...
! Initialize MPI with threading
       ...
       call MPI_COMM_SIZE(MPI_COMM_WAVE, nprocs, ierr)
       call mpi_comm_rank(MPI_COMM_WAVE, myrank, ierr)

! WW3 used number of processors and rank id
       NAPROC = nprocs 
       IAPROC = myrank + 1

! Cell files are now copied to $SCRDIR before run.
       CALL get_environment_variable("SCRDIR", CelPath)

!  Setup allocatable arrays for broadcast among ranks
       ALLOCATE( INTALLOC(14), INTALLO2(0:MRL, 3), STAT=malloc )

!  Read Global and Arctic part Multiple-Cell info on rank 0 
       if (myrank .eq. 0) then

!  Timing the program from start.   
          CALL DATE_AND_TIME(CDate, CTime)
          GTM0 = MPI_WTIME()
          WRITE(6,*) ' Read cell files from path: ', CelPath
          WRITE(6,*) " Required, provided threading support: ",   &
      &                required, provided
...
!  Broadcast cell and face parameters to all ranks
       CALL MPI_Bcast(INTALLOC, 14, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)
       m = (MRL+1)*3
       CALL MPI_Bcast(INTALLO2,  m, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)
...
! Broadcast cell and face arrays among all ranks.
       n=5*(NCL+10)
       CALL MPI_Bcast(ICE(1,-9), n, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)
!      m=7*NFC
!      CALL MPI_Bcast(ISD(1, 1), m, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)
       m=5*NFC
       CALL MPI_Bcast(ISD(3, 1), m, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)
       k=6*NFC
       CALL MPI_Bcast(JSD(2, 1), k, MPI_INTEGER, 0, MPI_COMM_WAVE, ierr)

!      WRITE(6,*) ' Cell Array Bcast done on ', IAPROC
!      FLUSH(6)
       CALL MPI_BARRIER(MPI_COMM_WAVE, ierr)
...
!       WRITE(6,*) ' Cell preparation done on ', IAPROC
!       FLUSH(6)
...
!!  Setup communication links and assign spectral components to ranks.
        CALL W3INIT
!       WRITE(6,*) ' W3INIT done for rank ', IAPROC
!       FLUSH(6)
...
!  Initialise AngCD/ELaCD to be zero.
        AngCD=0.0
        ELaCD=0.0

!  Generate Arctic boundary cell matching pointers and 
!  cell rotation Angles from map-east to local east. 
        IF( Arctic )  THEN
            CALL ArctAngd
            ALLOCATE( SPCBAC(NSpc, NBAC), BACSPEC(NSpc) )
        END IF

!  Initialise wave spectra and great circle turning ratios.
        CALL SPECUUVV
...
!  Output wave height at start time, using new WW3 method.
!                        JGLi06Aug2019 

!  Save a copy of initial values at writeup time step.
!  New sub WRITESWH requires all PEs to calculate their
!  local HS(NSEALM) before last PE gathers them and output.
        NT = NS
        CALL WRITESWH( NT ) 
...
!!Li  Initialise n in case OpenMP is not used.
        nthreads = 1
!$      TM1= OMP_GET_WTIME()
...
!     Header messages and configuration information 
       ...
       WRITE(UNIT=16,FMT='(1x," Globl/Arctic bndy No.s = ",2i8)')  NBGL, NBAC
       ...
       WRITE(UNIT=16,FMT='(1x," Spctr/Sea pts per rank = ",2i8)')  NSPLOC, NSEALM
 ...
 3912 FORMAT(1x,i4,3F9.1,ES12.3)
       ...
       WRITE(16,FMT='(1x,8i8)')  1,(ISD(i, 1),i=3,7)
       WRITE(16,FMT='(1x,8i8)') NU,(ISD(i,NU),i=3,7)
       WRITE(16,FMT='(/1x," First and last JSD values ")' ) 
       WRITE(16,FMT='(1x,9i8)')  1,(JSD(i, 1),i=2,7)
       WRITE(16,FMT='(1x,9i8)') NV,(JSD(i,NV),i=2,7)
       WRITE(16,FMT='(1x,8i8)') 
...
!$ACC         DCXDY,DCXDX,DCYDY,ESC,EC2,ES2,DHDX,ECOS,DHLMT,dsip,cx,cy,DHDY,C,D,  &
!$ACC         esin,sig,rcela,angcd)


 TSLoop:  DO  NT=NS, NS+NTS
 
        IF(myrank .eq. 0) WRITE(6,*) " Main time loop step NT = ", NT
...
#ifdef GPUDIRECT
!$acc host_data use_device(cx,cy)
#else
!$acc update host(cx,cy)
#endif
       CALL MPI_Bcast(CX(1), NSEA, MPI_REAL, 0, MPI_COMM_WAVE, ierr)
       CALL MPI_Bcast(CY(1), NSEA, MPI_REAL, 0, MPI_COMM_WAVE, ierr)
#ifdef GPUDIRECT
!$acc end host_data
#else
...
!!  Start gather/scatter communications for first component on each rank.
          IF ( NRQSG1 .GT. 0 ) THEN
...
!!  Select assigned spectral components for own rank.
  SpcLop:  DO NP=1, NSpc
              IF( IAPPRO(NP) .EQ. IAPROC ) THEN 
                  NF=( NP - 1)/NDir  + 1
                  ND=MOD(NP-1, NDir) + 1

!!  Gather spectral component into FIELD variable
                  CALL W3GATHSMC ( NP, FIELD )

!!  Propagation for the given spectral component
                  CALL W3PSMC( ND, NF, NT, FIELD )           

!!  Scatter spectral component FIELD back to VA 
                  CALL W3SCATSMC ( NP, FIELD )
...
!!    Find source boundary spectra and assign to SPCBAC
       DO K = 1, NBAC
          IF( K .LE. (NBAC-NBGL) ) THEN
              L = ICLBAC(K)
          ELSE
              L = NGLo + K 
          ENDIF 

!!    Work out root PE (ii) and local spectral number jj for L 
          ii = MOD( L-1, NAPROC ) 
          jj = 1 + (L-1-ii)/NAPROC

!!    Assign boundary cell spectra. 
          IF( IAPROC .EQ. ii+1 ) THEN
              SPCBAC(:,K)=VA(:,jj)
          ENDIF

!!    Broadcast local SPCBAC(:,IK) to all other PEs.
       CALL MPI_BCAST(SPCBAC(1,K),NSPEC,MPI_REAL,ii,MPI_COMM_WAVE,IERR_MPI)
       CALL MPI_BARRIER(MPI_COMM_WAVE, ierr)
 
!!    Boundary cell loop K ends.
       END DO


!!    Update Arctic boundary cell spectra if within local range
       DO K = 1, NBAC
          IF( K .LE. (NBAC-NBGL) ) THEN
              L = NGLo + K 
              BACANGL = ANGARC(K)
...
!!    Work out boundary PE (ISPEC) and JSEA numbers for IX 
          ii = MOD( L-1, NAPROC ) 
          jj = 1 + (L-1-ii)/NAPROC
!
          IF( IAPROC .EQ. ii+1 ) THEN
              BACSPEC = SPCBAC(:,K)
              CALL W3ACTURN( NDir, NFrq, BACANGL, BACSPEC )
              VA(:,jj) = BACSPEC 
          END IF
...
!      WRITE(6,*) "Arctic part is updated at ", NT
...
!!    Wait for NAPFLD rank finish its output. 
       CALL MPI_BARRIER(MPI_COMM_WAVE, ierr)
...
!!  End of time step loop
      ENDDO  TSLoop
       ...
       WRITE(UNIT=16,FMT="(1x,' Net Loop time s ',F10.1)") GTM2 - GTM1
       WRITE(UNIT= 6,FMT="(1x,' Net Loop time s ',F10.1)") GTM2 - GTM1
...
!!  End of rank 0 write out CTIME.
       ...
       IF(myrank .EQ. 0 .OR. myrank .EQ. nprocs-1 )  &
          WRITE(6,*) 'SMCPrDRG completed from rank ', myrank
...
!   flow field, using the same grid and time step.
      ...
      SUBROUTINE SPECUUVV
        ...
        INTEGER:: i, j, k, L, M, N, ii, ij, jk, kL, LL, MM, mn, NN
        INTEGER:: ISP, IK, ITH
...
!! Whole array assignment for VA(NSPEC,NSEALM)
        VA=0.0
...
!! upperlimit for the south and lowerlimit for the north strips. JGLi09Jul2019
      ...
      jk=NINT( (SpNorLat-ZrLAT)/DLat)
      kL=NINT( (70.00   -ZrLAT)/DLat)
      mn=NINT( (86.00   -ZrLAT)/DLat)

!! Loop over all cells to select strip points. 
!! Loop over local collection of sea points.  JGLi16Aug2019
!     DO i=1,NC
      DO ii=1,NSEAL
         VSpc = 0.0 
         i = MIN( IAPROC+(ii-1)*NAPROC, NSEA )
         ll=ICE(2,i)
         mm=ICE(2,i)+ICE(4,i)
...
!!  South and polar patch use Spec2D
         IF( mm .LT. ij .OR. mm .gt. mn) THEN
             VSpc(1:NDir,j)=Spec2D*Spec2F(j)
         ENDIF
!!  Northern boundary use Spec1D
         IF( jk .LT. ll .AND. ll .LT. kL ) THEN
             VSpc(1:NDir,j)=Spec1D*Spec1F(j)
!            C(i)=SWH0
         ENDIF

         ENDDO
!!  Store to local spectral variable. 

         DO ISP=1,NSpc
            ITH    = 1 + MOD(ISP-1,NDir)
            IK     = 1 + (ISP-1)/NDir
            VA(ISP,ii) = Vspc(ITH,IK)
         ENDDO
...
! 999  PRINT*, ' Sub SPECUUVV ended for rank ', IAPROC
...
! Subroutine Interface:
      ...
      Subroutine Specturn( NDirc, NFreq, Alphad, Spectn )
...
! Subroutine arguments
       ...
       REAL, INTENT(INOUT) :: Spectn(NDirc,NFreq)  ! Wave spectrum in and out
...
! Positive or anticlock case, larger bin upstream
      ...
      DO kk=NDirc, 1, -1
         ...
         Spectn(kk,:)=Wrkspc(kk,:) - Wrkfrq + Tmpfrq 
         Tmpfrq=Wrkfrq
      ENDDO
        ELSE
...
!!  For WW3 style spectral memory arrangement, Acrtic part MPI communication
!!  has to be updated to use temporary space to store boundary spectra.
        ALLOCATE( ANGARC(NArc) )
        ALLOCATE( ICLBAC(NBAC) )

!!  Mapping Arctic boundary cells with inner model cells
        DO L=1, NBAC 
             i=ICE(1,L+NGLo)
             j=ICE(2,L+NGLo)
           DO M=1, NGLo 
       ...
       &          (j .EQ. ICE(2,M)) ) THEN 
        ...
        ENDDO
!       WRITE(6,        *)  " NBAC = ", NBAC
!       WRITE(6,'(10i8)') (ICLBAC(ix), ix=1,NBAC,NBAC/8)
...
!!  will be negeligible as only the AnglD is used.
         XLon(L)= REAL( ICE(1,i) )*DLon + CNST1*REAL( ICE(3,i) ) + ZrLon 
         WLat(L)= REAL( ICE(2,i) )*DLat + CNST2*REAL( ICE(4,i) ) + ZrLat

      END DO

!! Although north polar cell centre coincides with NP, its rotation angle
!! has to be evaluaged at its edge for rotation purpose.  JGLi11Nov2019 
         XLon(NArc)=0.0
!        WLat(NArc)=90.0
         WLat(NArc)=REAL( ICE(2, NC) )*DLat + ZrLat
...
!  Last modified:    1 Apr 2015   Jian-Guo Li
      ...
      SUBROUTINE ReadCell
        ...
        INTEGER :: i, j, k, L, M, N, ii, jj, kk, LL, MM, NN, LMN
        REAL:: CNST, CNST0, CNST1, CNST2, CNST3, CNST4, CNST5, CNST6
...
!!  Total Arctic boundary cells redefined by NBAC
          NBAC=NBAC+NBGL
          PRINT*, ' With Arctic part', NArc, NBAC, NBGL

       ELSE
          NArc = 0
          NBAC = 0
          NBGL = 0
          PRINT*, ' No Arctic part', NArc, NBAC, NBGL
       ENDIF
...
!!   Boundary cell will then from NGLo-NBGL+1 to NGLo for global part and 
!!   NGLo+1 to NGLo-NBGL+NBAC for the Arctic part.
...
!        READ(10,*)  (ISD(N,I+NUGL), N=1,7)
         READ(10,*)  ii, jj, (ISD(N,I+NUGL), N=3,7)
      END DO
      CLOSE(10)
...
!        READ(11,*)  (JSD(N,J+NVGL), N=1,8)
         READ(11,*)  ii, (JSD(N,J+NVGL), N=2,7), kk
      END DO
      CLOSE(11)
...
!  Last modified:   15 Aug 2019   Jian-Guo Li
      ...
      SUBROUTINE WRITESWH( MT )
        ...
        INTEGER::  i, j, k, n, jk, nn 

!!    First calculate SWH on each rank.
        HS(:)=0
!       WRITE(6, *) " NSEAL and rank are ", NSEAL, IAPROC

!$OMP Parallel DO Default(Shared), Private(i,j,k,jk, CTT, CMX) 
        DO i=1, NSEAL
           CTT=0.0
           DO k=1, NSPEC
              jk = 1+(K-1)/NDir 
              CTT = CTT + VA(k,i)*DSIP(jk)
           ENDDO
           CMX=CTT*DThta
...
!  Wait for all PEs to finish local HS calculation.
         CALL MPI_BARRIER(MPI_COMM_WAVE, ierr)

!$OMP MASTER

!  Then start sending HS to rank NAPFLD with preset send. 
        IF ( IAPROC .NE. NAPFLD ) THEN
           ALLOCATE( STATIO(MPI_STATUS_SIZE) )
           CALL MPI_START ( IRQGO, IERR_MPI )
           CALL MPI_WAIT  ( IRQGO, STATIO, IERR_MPI )
           DEALLOCATE ( STATIO )
        END IF

!  Collect HS into XHS on rank NAPFLD with preset recv.
        IF ( IAPROC .EQ. NAPFLD ) THEN
           ALLOCATE( STATI2(MPI_STATUS_SIZE, NRQGO2) ) 
!          WRITE(6,*) " Start NRQGO2 on rank ", NRQGO2, NAPFLD
           CALL MPI_STARTALL ( NRQGO2, IRQGO2, IERR_MPI )
           CALL MPI_WAITALL  ( NRQGO2, IRQGO2, STATI2, IERR_MPI )
!          WRITE(6,*) " Done Waitall NRQGO2 at NT ",   MT
           DEALLOCATE ( STATI2 )

!  Append the last rank's HS to XHS.   JGLi28Oct2019
           DO i=1, NSEAL
              n = NAPFLD+(i-1)*NAPROC
              XHS( n ) = HS( i )
           END DO
...
!    All cells are saved 
           ...
           WRITE(UNIT=26, FMT=7113)  (XHS(n),  n=1, NC)
           CLOSE(26)
 7113 FORMAT( 1x, 7ES11.3 )

!!  End of rank NAPFLD output.
        END IF
