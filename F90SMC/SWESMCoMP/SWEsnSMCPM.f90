...
!!  with initial value at 1/5 of the maximum one.  JGLi09Jan2024
        AKHDMX= 2.0*AKHM*DT/(REARTH*REARTH)
        AKHDT2= 0.2*AKHDMX    !! Suspended 04Mar2024
...
!!  Default ocean floor bottom altitude at -KG    JGLi06Jun2016
        ...
        WRITE(6,FMT='(" Maximum height above sea level=",f8.1)') MINVAL(Btm(1:NC))
...
!! Open a file to store water height at selected points if WHPrts is selected.
       IF( WHPrts ) THEN
        ...
        &           ACTION='WRITE')
         ...
         WRITE(20,FMT='(1x,i7,(36i8))') NPrt, (IDPrt(i), i=1, NPrt)
!  Output initial error norms at 0 time step. 
         WRITE(20,FMT=7113) 0,(Hw(IDPrt(i))+MIN(Btm(IDPrt(i)),0.0),i=1,NPrt)
       ENDIF
!7113  FORMAT (1x,i7, (18ES11.3))
 7113  FORMAT (1x,i7, (36F8.3))
...
!  Update AKHDT2 with ramp factor upto 24*Mhr.   JGLi05Mar2024
          CNST = 0.2+0.8*NT/FLOAT(24*MHr)
          AKHDT2 = MIN(1.0, CNST)*AKHDMX

        ENDIF

        IF( WHPrts .AND. MOD(NT, 2) .eq. 0 )  THEN 
          WRITE(20,FMT=7113) NT,(Hw(IDPrt(i))+MIN(Btm(IDPrt(i)),0.0),i=1,NPrt)
        ENDIF
